...
export default function PreviewPanel({ html, template, status, hasContent }: PreviewPanelProps) {
  return (
    <section className="flex min-h-[36rem] flex-col gap-3 rounded-lg border border-slate-200/80 bg-white p-3.5 shadow-sm shadow-slate-200/50 md:p-4">
      <div className="relative z-10 flex min-h-12 shrink-0 flex-wrap items-start justify-between gap-2 border-b border-slate-100 pb-3">
        <div className="min-w-0">
          <h2 className="text-base font-semibold leading-6 text-slate-950">排版预览</h2>
        ...
        {status ? (
          <div className="rounded-md bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700 ring-1 ring-emerald-100">
            {status}
          </div>
        ) : null}
      </div>

      <div className="preview-surface min-h-0 flex-1 overflow-auto rounded-md border border-slate-200 bg-slate-100/70 p-3 shadow-inner md:p-4">
        <div className="mx-auto min-h-full max-w-[48rem] rounded-sm border border-slate-200 bg-[#fffefb] p-5 shadow-[0_18px_42px_rgba(15,23,42,0.12)] md:p-8">
          {hasContent ? (
